...
# selected variables
# throughout trying to rename variables to reflect table names
# so they're easier to understand in joined tables
...
# filter to only products for inhalation
dispensing <- filter(dispensing, inventorytype==24)
dispensing$saletime <- as.Date(as.POSIXct(dispensing$sessiontime, 
                                           origin = "1970-01-01", tz="America/Los_Angeles"))
# select only the variables we need
dispens.select <- dispensing %>%
  dplyr::select(dispensingid = id, retail_invid = inventoryid, retail_loc = location,
...
# 5% of product names for inhalants are missing, which is ~ the same proportion as in full inventory df
...
# get a df that is for each unique inhalant name, the boolean values for classifying
...
# get summary of inhalant classifications
prelimCat <- inhalants %>%
...
# running this based on product names, not actual count of products in dispensing
