...
# Key findings
 ...
 - Able to perform some form of **classification for about 60%** of the products for inhalation in the dispensing file.
 - Including processors' inventory types in the process should get the number of **unclassified inhalants down to about 17%**.
 - Need to review these preliminary categories from a domain standpoint, and to consider purposes of classification.
 - May need stricter categorization for about 5% of products that match more than one keyword. 
...
test.retaillocations <- dispens.select %>%
  ...
  dplyr::select(dis_loc = retail_loc, dis_name = name, dis_city = city, dis_loctype = loc_type.x, retail_invid,
         -(sale_price), -(saletime)) %>%
...
avg.combos <- test.combos %>%
  ...
  dplyr::ungroup() %>%
  dplyr::summarise(avg.combos = mean(count, na.rm=T))
```
...
```{r prelimClass, echo=FALSE, warning=FALSE, cache=TRUE}
...
knitr::kable(prelimCat, col.names = c("Keyword", "Percent", "Count"),
               align=c("r", "r", "r"))
```
...
inhalants %>%
  ...
  dplyr::group_by(keyword) %>%
  dplyr::summarise(perc.inhalants = round(sum(status==TRUE) / nrow(inhalants)*100,2),
                   perc.cart = round(sum(status==TRUE) / cart_count[1] * 100,2),
                   count = sum(status==TRUE)) %>%
  dplyr::arrange(desc(perc.cart)) %>%
  knitr::kable(col.names = c("Keyword", "% of Inhalants", "% of Cartidges", "Count"),
               align=c("r", "r", "r", "r"))
