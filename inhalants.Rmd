...
overlap.oil <- inhalants %>%
  ...
  dplyr::select(oil_count, dispensingid, cartidge, BHO:wax) %>%
  # gather to get row identified + column of keywords + column of T/F values
  tidyr::gather(keyword, status, cartidge, BHO:wax) %>%
...
# Next Steps
 ...
 - Possibly aggregage back up into more generalized inhalant categories like "cartidge", "concentrate", "other", or something else that more generally captures either use-type, potency cluster, or both
