...
extracts$inventoryid <- as.numeric(extracts$inventoryid)
...
potency_tidy <- potency %>%
  ...
  dplyr::left_join(select(labkey, sample_id = id, inventoryid), by="sample_id") %>%
  dplyr::ungroup() 


tests <- tests %>%
  dplyr::left_join(select(labkey, sample_id = id, inventoryid), by="sample_id") %>%
  dplyr::group_by(inventoryid, name) %>%
  ...
  dplyr::left_join(potency_tidy, by="inventoryid")
...
tests_extracts <- extracts %>%
  dplyr::select(inventoryid, price_x, price_per_gram,
                inhalant_type, inhalant_gen, productname) %>%
  dplyr::left_join(tests_numeric, by="inventoryid")
...
tests_extracts_joined <- tests_numeric %>%
...
tests_extracts_joined <- tests_numeric %>%
  dplyr::left_join(select(extracts, inventoryparentid, price_x, price_per_gram,
                          inhalant_type, inhalant_gen, productname),
                   by=c("inventoryid" = "inventoryparentid"))
