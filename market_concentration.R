...
producttypes <- as.data.frame(unique(trans.select$trans_invtypename))
colnames(producttypes) <- "trans_invtypename"
# use categorization function to get grouping for each product type
producttypes <- producttypes %>%
  rowwise() %>%
  dplyr::mutate(product_group = groupProcessorProductTypes(trans_invtypename))
# join back to transfers df
trans.select$trans_invtypename <- as.factor(trans.select$trans_invtypename)
trans.select <- left_join(trans.select, producttypes, by="trans_invtypename")
